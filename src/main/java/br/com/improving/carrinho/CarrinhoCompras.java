...
public class CarrinhoCompras {
	...
	public void adicionarItem(Produto produto, BigDecimal valorUnitario, int quantidade) {
		...
		try {
			...
			Optional<Item> ItemExiste = Optional.ofNullable(getItens().stream()
					.filter(Items -> Items.getProduto().equals(produto)).findFirst().orElse(null));

			if (ItemExiste.isPresent()) {
				Item item = ItemExiste.get();
				somarQuantidadeItens(item, quantidade);
	...
	public BigDecimal getValorTotal() {
		return BigDecimal.valueOf(
				getItens().stream().mapToDouble(item -> item.getValorTotal().doubleValue()).sum());
	}
	...
	public void somarQuantidadeItens(Item item, int quantidade) {
	...
	/**
	 * Exclui todos itens do carrinho de uma única vez
	 *
     * @return Retorna um boolean
	 *  
	 */
	public boolean removerTodosItensCarrinho() {
		ArrayList<Item> itens = (ArrayList<Item>) getItens();
		return getItens().removeAll(itens);
	}
}